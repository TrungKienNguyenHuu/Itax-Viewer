...
import seatechit.tool.ConfigInfo;
import seatechit.tool.ITaxViewerException;
import seatechit.tool.SWidgetTool;
import seatechit.tool.TaxViewExceptionHandler;
import seatechit.tool.signature.XMLSignatureValidationResult;
import seatechit.tool.taxdoc.HSoThue;
import seatechit.tool.taxdoc.HSoThueFactory;
import seatechit.tool.taxdoc.ITaxInvalidDocException;
import seatechit.tool.taxdoc.PLuc;
...
public class HSoThueViewer
{
  ...
  public HSoThueViewer(ConfigInfo config, Composite viewArea, Composite actionArea, String tkhaiFileName) throws IOException, ParserConfigurationException, ITaxInvalidDocException, ITaxViewerException {
    HSoThueViewer.viewArea = viewArea;
    HSoThueViewer.actionArea = actionArea;
    hso = HSoThueFactory.createHSoThue(tkhaiFileName, config);
  }

  public void createHSoViewer() throws IOException {
    if ("1".equals(hso.getViewMethod())) {
      closeHTMLViewer();
      createHTMLViewer();
  ...
  public void displaySignatureStatus() throws Exception {
    Collection validationResult = hso.getSigValidationResult();
  ...
  private String getSigStatusHTML(HSoThue hso) throws IOException { Collection validationResults = hso.getSigValidationResult();
    ...
    while (ito.hasNext()) {
      ...
      String sigImageURL;
      if (XMLSignatureValidationResult.SIG_STATUS_GOOD == valResult
        .getValidStatus()) {
        ...
        sigStatusMsgColor = "red";
          sigImageURL = new File("image\\error_sig.png").toURI()
                  .toURL().toString();
      ...
      msgHTML = msgHTML + "<tr>" + " <td style=\"vertical-align:middle\"><img src=\"" + sigImageURL + "\"/></td>" + " <td style=\"vertical-align:middle;font-family:arial;color:" + sigStatusMsgColor + "\">" + sigStatusMsg + ".</td>" + "</tr>";
    }
  ...
  public void printFile() throws IOException {
    ...
    try {
      ...
      if (objPDFRunable) {
          script = 
              "var objPDF= '<object id=\"tkhaiPDF\" width=\"0\" height=\"0\" onreadystatechange=\"this.printWithDialog();\" type=\"application/pdf\" data=\"" + 
              tkhaiPDFURL + "\"></object>';\n" + 
          ...
          if (tempfile.exists() && tempfile.canRead()) {
              ...
              throw new IOException("PDF file is not accessible: " + tempfile.getPath());
          }
      }
    } catch (Exception ex) {
      ex.printStackTrace();
      throw new IOException("Error opening PDF: " + ex.getMessage());
    } finally {
        // Clean up temporary files in a separate thread
        new Thread(() -> {
            try {
                Thread.sleep(2000); // Wait before attempting to delete
                for (File file : printFiles) {
                    try {
                        file.delete();
                    } catch (Exception e) {
