...
public class IHTKKXMLSignature {
    ...
    public void signXMLDoc(X509Certificate cert, PrivateKey privateKey, Document doc) throws Exception {
        ...
        SignedInfo si =
                fac.newSignedInfo(fac.newCanonicalizationMethod("http://www.w3.org/TR/2001/REC-xml-c14n-20010315#WithComments", (XMLStructure) null),
  ...
  /*public Collection verifyXMLSignature(Document doc) {
    CertVerifier certVerifier = new CertVerifier(rootCerts, trustedCerts);
    ...
    try {
      ...
      else
      {
        ...
        for (int i = 0; i < signl.getLength(); i++) {
            result = new XMLSignatureValidationResult();
            try {
                try {
                ...
                if ((e instanceof ITaxSigValidException)) {
                    result.setValidStatus(XMLSignatureValidationResult.SIG_STATUS_ERROR);
                } else if ((e instanceof ITaxCertValidException)) {
                    result.setValidStatus(XMLSignatureValidationResult.SIG_STATUS_ERROR);
                } else if ((e instanceof ITaxStatusValidException)) {
                    result.setValidStatus(XMLSignatureValidationResult.SIG_STATUS_WARNING);
                }
                result.setValidMessage(e.getMessage());
            }
        }
    ...
    public Collection<XMLSignatureValidationResult> verifyXMLSignature(Document doc) {
        CertVerifier certVerifier = new CertVerifier(rootCerts, trustedCerts);
        ArrayList<XMLSignatureValidationResult> resultArr = new ArrayList<>();
        X509Certificate[] certChain = null;

        XMLSignatureFactory fac = XMLSignatureFactory.getInstance("DOM");

        NodeList signl = doc.getElementsByTagNameNS(
                "http://www.w3.org/2000/09/xmldsig#",
                "Signature"
        );
        System.out.println("Found " + signl.getLength() + " Signature elements");
        ...
        try {
            ...
            String nodeDataSig = elemData[elemData.length - 1];
            NodeList nodes = doc.getElementsByTagName(nodeDataSig);
        ...
        // Always process the signatures we found
        for (int i = 0; i < signl.getLength(); i++) {
            ...
            try {
                try {
                    Element tempNode = (Element) signl.item(i);
                    Element timeStamp = (Element) tempNode.getElementsByTagName("DateTimeStamp").item(0);
                    result.setTimeStamp(timeStamp != null ? timeStamp.getAttribute("DateTime") : "");
                } catch (Exception e) {
                    result.setTimeStamp("");
                }

                DOMValidateContext valContext = new DOMValidateContext(new X509KeySelector(), signl.item(i));
                if (!checkSignXpath && nodeToSign != null) {
                    valContext.setIdAttributeNS((Element) nodeToSign, null, "id");
                }
                ...
                result.setValidStatus(XMLSignatureValidationResult.SIG_STATUS_GOOD);
            } catch (Exception e) {
                if (e instanceof ITaxSigValidException || e instanceof ITaxCertValidException) {
                    result.setValidStatus(XMLSignatureValidationResult.SIG_STATUS_ERROR);
                } else if (e instanceof ITaxStatusValidException) {
                    result.setValidStatus(XMLSignatureValidationResult.SIG_STATUS_WARNING);
                }
    ...
    public Collection verifyXMLSignature(Document doc, String xPath_data) {
        ...
        try {
            ...
            if (nodes.getLength() > 0) {
                ...
                int i = 0;
                do {
                    String attNamr = atts.item(i) != null ? atts.item(i)
                            .getNodeName() : null;
    ...
    private X509Certificate[] getCertificateChain(KeyInfo keyInfo) throws Exception {
        ...
        System.out.println("Processing KeyInfo...");
        List<XMLStructure> keyInfoContent = keyInfo.getContent();

        for (XMLStructure keyInfoStructure : keyInfoContent) {
            System.out.println("KeyInfo structure type: " + keyInfoStructure.getClass().getName());

            if (keyInfoStructure instanceof X509Data) {
                X509Data x509Data = (X509Data) keyInfoStructure;
                List<?> x509DataContent = x509Data.getContent();

                System.out.println("X509Data content size: " + x509DataContent.size());

                for (Object x509DataObject : x509DataContent) {
                    System.out.println("X509Data object type: " + x509DataObject.getClass().getName());

                    if (x509DataObject instanceof X509Certificate) {
                        X509Certificate cert = (X509Certificate) x509DataObject;
                        System.out.println("Found certificate: " + cert.getSubjectX500Principal().getName());
                        certList.add(cert);
                    }
                }
            }
        ...
        if (certList.isEmpty()) {
            System.out.println("No certificates found in KeyInfo");
            return null;
        }

        System.out.println("Found " + certList.size() + " certificates");
