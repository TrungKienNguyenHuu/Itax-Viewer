package seatechit.tool.taxmessagekdtt;
...
import seatechit.tool.signature.IHTKKXMLSignature;
  ...
  extends HSoThue
{
    ...
    throws IOException, ParserConfigurationException, ITaxViewerException
  {
    ...
    xsdFile = dmTBaoNtdt.getXSDTBao(maHSo, pbanHSoXML);
    if (xsdFile == null) {
      throw new ITaxViewerException("Mã Thông báo (" + maHSo + 
        ") hoặc phiên bản thông báo (" + pbanHSoXML + 
        ") không đúng");
    }
    
    viewMethod = dmTBaoNtdt.getTBaoViewType(maHSo, pbanHSoXML);
    
    excelFile = dmTBaoNtdt.getExcelTemplateTTBao(maHSo, pbanHSoXML);
    
    xsltFile = dmTBaoNtdt.getXSLTTBao(maHSo, pbanHSoXML);
    
    orientation = dmTBaoNtdt.getTKhaiOrientation(maHSo, pbanHSoXML);
    
    plucList = getPLucList(dmTBaoNtdt);
  ...
  private Collection getPLucList(DMucTBaoKDTT dmTBao) {
    ...
    for (int i = 0; i < len; i++) {
      if (plucList.item(i).getNodeType() == 1) {
        ...
        pluc.setPlucViewMethod(dmTBao.getPLucViewType(maHSo, 
          pbanHSoXML, plucID));
        pluc.setOrientation(dmTBao.getPLucOrientation(maHSo, 
          pbanHSoXML, plucID));
        pluc.setPlucExcelFile(dmTBao.getExcelTemplatePLuc(maHSo, 
          pbanHSoXML, plucID));
        pluc.setPlucXSLTFile(dmTBao.getXSLTPLuc(maHSo, pbanHSoXML, 
          plucID));
