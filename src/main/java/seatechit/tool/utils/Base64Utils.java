...
public class Base64Utils
{
  ...
  public static String base64Encode(byte[] aData)
  {
    ...
    for (int destIndex = 0; srcIndex < aData.length - 2; srcIndex += 3) {
      ...
      encodedBuf[(destIndex++)] = mBase64EncMap[
        (aData[(srcIndex + 1)] >>> 4 & 0xF | aData[srcIndex] << 4 & 0x3F)];
      encodedBuf[(destIndex++)] = mBase64EncMap[
        (aData[(srcIndex + 2)] >>> 6 & 0x3 | aData[(srcIndex + 1)] << 2 & 0x3F)];
      encodedBuf[(destIndex++)] = mBase64EncMap[(aData[(srcIndex + 2)] & 0x3F)];
  ...
  public static byte[] base64Decode(String aData)
  {
    ...
    for (int destIndex = 0; destIndex < decodedBuf.length - 2; destIndex += 3) {
      ...
      decodedBuf[(destIndex + 2)] = ((byte)(data[(srcIndex + 2)] << 6 & 0xFF | data[(srcIndex + 3)] & 0x3F));srcIndex += 4;

      if (destIndex < decodedBuf.length) {
        decodedBuf[destIndex] = ((byte)(data[srcIndex] << 2 & 0xFF | data[(srcIndex + 1)] >>> 4 & 0x3));
      }
      destIndex++; if (destIndex < decodedBuf.length) {
